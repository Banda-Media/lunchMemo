...
class LunchGroupRow {
  ...
  btnJoinLeavePressed() {
    if (this.groupObj.users.includes(JSON.parse(window.sessionStorage.me)._id)) {
      this.groupObj.users.splice(
        this.groupObj.users.indexOf(JSON.parse(window.sessionStorage.me)._id),
        1
      );
      this.joinLeaveBtn.className = 'btn btn-join';
      this.joinLeaveBtn.innerHTML = 'Join';
  ...
  async updateAttendeesView() {
    ...
    Array(this.attendeeUL.childNodes.length)
      .fill()
      .map((_, i) => i >= maxSize && this.attendeeUL.childNodes[i].remove());
